...
def load_kmnist(args):
    ...
    original_train = dsets.KMNIST(root=args.data_dir, train=True, download=True, transform=transforms.ToTensor())
    ori_data, ori_labels = original_train.data, original_train.targets.long()
    
    test_dataset = dsets.KMNIST(root=args.data_dir, train=False, download=True, transform=test_transform)
